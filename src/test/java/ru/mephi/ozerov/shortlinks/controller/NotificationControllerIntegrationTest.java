...
class NotificationControllerIntegrationTest {
    ...
    void whenClickLimitReached_notificationAppearsInList() throws Exception {
        ...
        String shortCode = objectMapper.readTree(createResult.getResponse().getContentAsString())
                .get("link").get("shortCode").asText();

        mockMvc.perform(get("/" + shortCode).header("X-User-Id", userId.toString())).andExpect(status().is3xxRedirection());
        mockMvc.perform(get("/" + shortCode).header("X-User-Id", userId.toString())).andExpect(status().isGone());
    ...
    void markAsRead_returns204() throws Exception {
        ...
        ).get(0).get("shortCode").asText();
        mockMvc.perform(get("/" + shortCode).header("X-User-Id", userId.toString())).andExpect(status().is3xxRedirection());

        MvcResult notifResult = mockMvc.perform(get("/api/notifications").header("X-User-Id", userId.toString()))
                .andExpect(status().isOk())
                .andReturn();
