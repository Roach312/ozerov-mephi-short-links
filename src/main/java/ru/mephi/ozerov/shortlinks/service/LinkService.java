...
public class LinkService {
    ...
    /**
     * Создаёт короткую ссылку. Уникальный shortCode для каждого вызова (разные пользователи — разные ссылки).
     */
    @Transactional
    ...
    /**
     * Находит ссылку по shortCode и увеличивает счётчик переходов.
     * Если лимит исчерпан после перехода — создаёт уведомление и деактивирует ссылку.
     */
    @Transactional
    ...
    public Optional<Link> update(Long id, UUID userId, String originalUrl, Integer clickLimit) {
        return findByIdAndUserId(id, userId).map(link -> {
            if (originalUrl != null && !originalUrl.isBlank()) link.setOriginalUrl(originalUrl);
