 ...
 * <ul>
 *   <li><b>Создание коротких ссылок:</b> система принимает длинный URL и преобразует его в короткую ссылку.
 *       Пример: при передаче https://www.baeldung.com/java-9-http-client вы получаете короткий вариант
 *       http://localhost:8080/3DZHeG</li>
 *   <li><b>Переход по короткой ссылке:</b> при вводе короткой ссылки в консоль пользователь перенаправляется
 *       на исходный ресурс в браузере: {@code Desktop.getDesktop().browse(new URI(...));}</li>
 * </ul>
 * <p>
 * Запуск (сервис должен быть запущен на http://localhost:8080):<br>
...
public class ShortLinkBrowserLauncher {
    ...
    private static final Pattern SHORT_URL_PATTERN = Pattern.compile("\"shortUrl\"\\s*:\\s*\"([^\"]+)\"");
    ...
    /** User-ID, полученный при создании короткой ссылки; используется при открытии ссылки в браузере (заголовок X-User-Id). */
    ...
    private static void printMenu() {
        ...
        System.out.println("2 — Открыть короткую ссылку в браузере (введите короткую ссылку или код)");
    ...
    private static void createShortLink(Scanner scanner, String baseUrl) {
        System.out.print("Введите длинный URL (например https://www.baeldung.com/java-9-http-client): ");
        ...
        if (shortUrl.isPresent()) {
            ...
            System.out.println("Не удалось создать короткую ссылку. Проверьте URL и что сервис запущен.");
        }
    }
     ...
     */
    private static Optional<String> createShortLinkViaApi(String baseUrl, String originalUrl, String userId) {
        ...
        try {
            ...
            if (statusCode != 201) {
                System.err.println("Сервер вернул код " + statusCode + (body != null && !body.isBlank() ? ": " + body : ""));
            ...
            String detail = (msg != null && !msg.isBlank()) ? msg : (e.getCause() != null && e.getCause().getMessage() != null ? e.getCause().getMessage() : e.getClass().getSimpleName() + " (проверьте, запущен ли сервис на " + baseUrl + ")");
    ...
    /**
     * Открытие короткой ссылки в браузере по ТЗ: ввод короткой ссылки в консоль → перенаправление на исходный ресурс в браузере.
     * Для перехода по ссылке сервер требует заголовок X-User-Id (владелец ссылки).
     */
    private static void openShortLinkInBrowser(Scanner scanner, String baseUrl) {
        ...
        if (userId == null || userId.isBlank()) {
            System.out.print("Введите ваш User-ID (UUID) или сначала создайте короткую ссылку (п.1): ");
        ...
        System.out.print("Введите короткую ссылку или код (например 3DZHeG или http://localhost:8080/3DZHeG): ");
        ...
        if (originalUrl.isEmpty()) {
            System.out.println("Ссылка недоступна: не найдена, истекла, исчерпан лимит или доступ запрещён (неверный User-ID).");
            return;
        }
    ...
    private static Optional<String> resolveOriginalUrl(String shortLinkUrl, String userId) {
        ...
        try {
            ...
            if (code == 403) {
                System.err.println("Доступ запрещён: ссылка принадлежит другому пользователю (403).");
                return Optional.empty();
            }
            if (code == 404) {
            ...
            if (code == 410) {
                System.err.println("Ссылка недоступна: истекла или исчерпан лимит переходов (410).");
                return Optional.empty();
            }
            return Optional.empty();
        } catch (Exception e) {
    ...
    /**
     * Открывает исходный ресурс в браузере по ТЗ: Desktop.getDesktop().browse(new URI(исходный_URL)).
     */
    private static void openOriginalResourceInBrowser(String originalUrl) {
        if (!Desktop.isDesktopSupported()) {
            System.out.println("Открытие браузера недоступно. Откройте ссылку вручную: " + originalUrl);
            return;
        }
        Desktop desktop = Desktop.getDesktop();
        if (!desktop.isSupported(Desktop.Action.BROWSE)) {
            System.out.println("Действие BROWSE недоступно. Откройте ссылку вручную: " + originalUrl);
            return;
        }
        try {
